 ...
 * listener shapes against the host contract — the host builds the execution
 * object with parsed tool arguments in the `arguments` field.
 * @module dsh-ci-doctor/events
 */

/** Minimal shape of the pending execution the waterfalls hand over. */
export interface PendingExecution {
  /** Registered tool name (e.g. `bash`, `ci_diagnose`). */
  name?: string
...
export type PostToolDecision = { kind: 'accept' } | { kind: 'block'; feedback: unknown }
